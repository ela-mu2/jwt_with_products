        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    <h2 className="text-lg font-bold text-slate-900">{editingProduct ? "Edit Product Details" : "Add New Inventory Product"}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 flex-1 overflow-y-auto space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="category" value={formData.category} onChange={handleChange} className="w-full px-3.5 py-2 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900">
                                {categories.map((cat) => (
                    ...
                    <div>
                        ...
                        <input type="url" name="imageUrl" value={formData.imageUrl} onChange={handleChange} placeholder="https://example.com" className="w-full px-3.5 py-2 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 mb-2" />

                        {formData.imageUrl && (
                            <div className="mt-2 p-2 border border-dashed border-slate-200 rounded-xl flex items-center gap-3 bg-slate-50/50">
                                <img src={formData.imageUrl} alt="Preview" className="w-12 h-12 object-contain bg-white rounded-lg border border-slate-100" onError={(e) => (e.target.style.display = "none")} />
